...
function main()
    ...
    if waitImageVisible(get_started, 8) then
        toast('get_started')
    ...
    if waitImageVisible(enter_the_confirmation_code) then
        ...
        local OTPcode = getThuemailConfirmCode()
        if OTPcode then 
            press(130, 410)
            press(660, 450)
    ...
    if waitImageVisible(what_on_your_mind) then 
        ...
        if waitImageVisible(logout_red_btn) then
            findAndClickByImage(logout_red_btn) sleep(1)
            waitImageNotVisible(logout_red_btn)
        end
